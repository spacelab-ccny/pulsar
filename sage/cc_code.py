# This was generated by preprocessing
# sage --preprocess cc_code.sage
# mv cc_code.sage.py cc_code.py

# This file was *autogenerated* from the file cc_code.sage
...
class PolarCode:
    def constructR(self, n):
        ...
        N = _sage_const_2**n
        mats_to_c = [_sage_const_0] * (N)
        halfway_pt = int(N / _sage_const_2)
        for i in range(halfway_pt):
            first = zero_matrix(N, _sage_const_1)
            first[_sage_const_2 * i] = _sage_const_1
            mats_to_c[i] = first
            sec = zero_matrix(N, _sage_const_1)
            sec[_sage_const_2 * i + _sage_const_1] = _sage_const_1
    ...
    def constructB(self):
        ...
        I_2 = identity_matrix(GF(_sage_const_2), _sage_const_2)
        B = copy(I_2)
        for i in range(self._pow_of_two - _sage_const_1):
            B_part = I_2.tensor_product(B, subdivide=False)
            B = self.constructR(i + _sage_const_2) * B_part
        return B
    ...
    def __init__(self, power_of_two, err_rate, dsd_r):
        ...
        sh_en = -_sage_const_1 * (
            err_rate * log(err_rate, _sage_const_2)
            + (_sage_const_1 - err_rate) *
            log(_sage_const_1 - err_rate, _sage_const_2)
        )
        N = int(_sage_const_2**power_of_two)
        self._N = N
        bin_cap = _sage_const_1 - sh_en
        ...
        heur_er = _sage_const_1 - bin_cap
        z_arr = self._N * [_sage_const_0]
        # fill the first slot
        z_arr[_sage_const_0] = heur_er
        for i in range(self._pow_of_two):
            real_ctr = int(_sage_const_2**i)
            for j in range(real_ctr):
                # you're going to be acting on two indices at once
                # real_ctr + j and j
                z_sq = z_arr[j] ** _sage_const_2
                z_arr[j] = _sage_const_2 * z_arr[j] - z_sq
                z_arr[j + real_ctr] = z_sq
        pi = [(i, z_arr[i]) for i in range(self._N)]
        for i in range(self._N):
            pne = pi.index(
                max(pi[: self._N - i], key=lambda x: x[_sage_const_1]),
                _sage_const_0,
                self._N - i,
            )
            tmp = pi[self._N - i - _sage_const_1]
            pi[self._N - i - _sage_const_1] = pi[pne]
            pi[pne] = tmp
        # how many do I take?
    ...
    def encode(self, msg):
        ...
        d = self._N * [_sage_const_0]
        ctr = _sage_const_0
        for i in range(self._N):
            if i in self._indices:
                d[i] = msg[ctr]
                ctr += _sage_const_1
        d_vec = vector(GF(_sage_const_2), d)
        out = d_vec * self.P
        return out  # * self.B

    # directly from [1], UpdateL function
    def updateL(self, l_m, d_m, row, col):
        s = _sage_const_2 ** (self._pow_of_two - col)
        s_half = int(s / _sage_const_2)
        l = int(row) % int(s)
        if l < s_half:
            if l_m[row][col + _sage_const_1] == NaN:
                self.updateL(l_m, d_m, row, col + _sage_const_1)
            if l_m[row + s_half][col + _sage_const_1] == NaN:
                self.updateL(l_m, d_m, row + s_half, col + _sage_const_1)
            a_p = l_m[row][col + _sage_const_1]
            b_p = l_m[row + s_half][col + _sage_const_1]
            ...
            if d_m[row - s_half][col] == _sage_const_0:
                # OLD LIKELIHOOD: l_m[row][col] = l_m[row][col+1]*l_m[row-s_half][col+1]
                l_m[row][col] = (
                    l_m[row][col + _sage_const_1]
                    + l_m[row - s_half][col + _sage_const_1]
                )
            else:
                # OLD LIKELIHOOD: l_m[row][col] = l_m[row][col+1] / l_m[row-s_half][col+1]
                l_m[row][col] = (
    ...
    def updateD(self, d_m, row, col):
        ...
        if l < s_half or col >= self._pow_of_two:
            ...
            d_m[row - s_half][col + _sage_const_1] = (
                d_m[row][col] + d_m[row - s_half][col]
            )
            d_m[row][col + _sage_const_1] = d_m[row][col]
            self.updateD(d_m, row, col + _sage_const_1)
            self.updateD(d_m, row - s_half, col + _sage_const_1)
        return
    ...
    def decode_to_message(self, word):
        ...
        L = []
        for i in range(self._N):
            new_r = [NaN] * (self._pow_of_two + _sage_const_1)
            L.append(new_r)
        D = []
        for i in range(self._N):
            new_r = [NaN] * (self._pow_of_two + _sage_const_1)
        ...
        BSC_TPM = [
            [_sage_const_1 - self._err_rate, self._err_rate],
            [self._err_rate, _sage_const_1 - self._err_rate],
        ]

        # first updating the last column of L
        for i in range(self._N):
            # L[i][-1] = BSC_TPM[0][word[i]] / BSC_TPM[1][word[i]]
            L[i][-_sage_const_1] = log(BSC_TPM[_sage_const_0][word[i]]) - log(
                BSC_TPM[_sage_const_1][word[i]]
            )
        ...
        for i in range(self._N):
            if i in self._indices:
                msg.append(D[i][_sage_const_0])
        return vector(GF(_sage_const_2), msg)
...
class ConcatenatedCode:
    def __init__(self, outer_code, inner_code, ic_name):
        ...
        # checking dimensions
        if inner_code.dimension() != int(
            math.log(outer_code.base_field().order(), _sage_const_2)
        ):
            raise ValueError(
                "IC dimension {} should be equal to log of OC field size {}".format(
    ...
    def from_file(cls, line_f):
        inner_c_name = line_f[-_sage_const_2]
        inner_c_details = line_f[-_sage_const_1]
        ic_len = int(line_f[-_sage_const_3])
        ic_dim = int(line_f[-_sage_const_4])
        inner_code = None
        if inner_c_name == "ReedMuller":
            num_vars = int(math.log(ic_len, _sage_const_2))
            inner_code = codes.ReedMullerCode(
                GF(_sage_const_2), int(inner_c_details), num_vars
            )
        elif inner_c_name == "BCH":
            inner_code = codes.BCHCode(
                GF(_sage_const_2), ic_len, int(inner_c_details))
        elif inner_c_name == "RandomLinear":
            # make a matrix from the list
            code_mtx = Matrix(GF(_sage_const_2), inner_c_details)
        ...
        oc_len = int(line_f[_sage_const_2])
        oc_dim = int(line_f[_sage_const_1])
        F = GF(_sage_const_2**ic_dim)
        list_elts = []
        if len(F) > _sage_const_4096:
            for z in range(i):
    ...
    def serialize(self):
        ...
        cstr = [
            rate,
            int(self._oc.dimension()),
            int(self._oc.length()),
            int(self._ic.dimension()),
            int(self._ic.length()),
            self._ic_name,
        ]
        if self._ic_name == "ReedMuller":
            ...
            raise ValueError(
                "Cannot serialize yet type {}. This is not supported :(".format(
                    self._ic_name
                )
            )
        return cstr
    ...
    def encode(self, msg):
        ...
        for symbol in outer_cw:
            as_bv = vector(GF(_sage_const_2), list(symbol))
            full_out.extend(self._ic.encode(as_bv))
        return vector(GF(_sage_const_2), full_out)

    def decode(self, codeword):
        # decode the inner code first
        bin_decoder = None
        if (
            self._ic_name == "ReedMuller"
            or self._ic_name == "BCH"
            or self._ic_name == "RandomLinear"
            or "Hamming"
        ):
        ...
        list_args = []
        for i in range(_sage_const_0, len(codeword), inner_len):
            word_as_vec = vector(GF(_sage_const_2), codeword[i: i + inner_len])
            idx = int(i / inner_len)
        ...
        with get_context("fork").Pool() as pool:
            ...
            out.sort(key=lambda elem: elem[_sage_const_0])
            for x in out:
                decoded_fl.extend(x[_sage_const_1])
            # decode_in_cw = bin_decoder.decode_to_message(word_as_vec)
            # decoded_fl.extend(decode_in_cw)
        bit_l = int(math.log(self._of.cardinality(), _sage_const_2))
        from_inner = [
            self._of(decoded_fl[i: i + bit_l])
            for i in range(_sage_const_0, len(decoded_fl), bit_l)
        ]
